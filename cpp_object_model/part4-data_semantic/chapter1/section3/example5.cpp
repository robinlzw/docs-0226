...
public:
	void printMemPoint() //因为要打印私有的成员变量偏移值，只能用成员函数打印
	{
		...
		printf("MYACLS::m_k = %d\n", &MYACLS::m_k); //MYACLS::m_k = 8
		printf("MYACLS::m_c = %d\n", &MYACLS::m_c); //MYACLS::m_c = 12
		printf("MYACLS::m_n = %d\n", &MYACLS::m_n); //MYACLS::m_n = 13
 
		printf("MYACLS::m_pria = %d\n", &MYACLS::m_pria); //MYACLS::m_pria = 17
		printf("MYACLS::m_prib = %d\n", &MYACLS::m_prib); //MYACLS::m_prib = 21
		cout << "------------------------" << endl;
		//利用GET宏定义来打印m_prib成员变量的偏移值
		cout << "MYACLS::m_prib = " << GET(MYACLS, m_prib) << endl; //MYACLS::m_prib = 21
	}
};
